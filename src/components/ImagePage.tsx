...
export default function ImagePage({ img, index, totalImages, plateLabel }: ImagePageProps) {
  return (
    <div className="relative w-full h-full bg-[#fbf9f6] select-none overflow-hidden">
      ...
      {/* Top frame */}
      <div className="absolute top-0 left-0 right-0 p-4 sm:p-6 flex items-start justify-between text-[10px] sm:text-xs font-sans-lux text-[#e8c896]/90 z-10 drop-shadow-md">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 sm:p-8 pb-12 sm:pb-16 z-10">
        <p className="font-serif italic text-[#ebe6dc] text-sm sm:text-lg leading-snug max-w-xl drop-shadow-md">
          {img.caption}
        </p>
        <div className="rule mt-3 sm:mt-4 max-w-[180px] bg-[#ebe6dc]/50" />
      </div>
    </div>
  );
}
